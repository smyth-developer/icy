            ...
            <table class=" min-w-full table-auto divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                <thead class=" text-gray-700 dark:text-gray-300 font-bold border-b text-xs uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left">Chương trình học</th>
                        <th class="px-6 py-3 text-center hidden 2xl:table-cell">Mô tả</th>
                ...
                <tbody id="sortable-program"
                    ...
                    @forelse ($programs as  $program)
                        <tr wire:key="role-{{ $program->id }}" data-id="{{ $program->id }}"
                            ...
                            <td class="px-3 py-2 text-gray-900 dark:text-white text-center">{{ $program->ordering }}
                            </td>
                            <td class="px-3 py-2 text-gray-900 dark:text-white">{{ $program->name }} ( <span
                                    class="font-bold text-pink-500">{{ $program->english_name }}</span> )</td>
                            <td class="px-3 py-2 text-gray-900 dark:text-white text-left hidden 2xl:table-cell">{{ $program->description }}
                            </td>       
                            <td class="px-3 py-2 text-center">
                                <div class="text-center gap-2">
                                    ...
                                    <flux:button class="my-0.5 cursor-pointer" variant="primary" color="pink"
                                        icon="trash" wire:click="deleteProgram({{ $program->id }})">Xóa
                                    </flux:button>
                                </div>
                            </td>
                        </tr>
