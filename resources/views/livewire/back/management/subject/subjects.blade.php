    ...
    <div class="mt-6">
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0" wire:key="table-{{ $selectedProgramId }}">
                <div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden" wire:key="table-container-{{ $selectedProgramId }}">
                    <div class="overflow-x-auto">
                        <table class="w-full divide-y divide-gray-200 dark:divide-gray-800" wire:key="table-inner-{{ $selectedProgramId }}">
                            <thead class="bg-gray-50 dark:bg-gray-800" wire:key="thead-{{ $selectedProgramId }}">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">Tên môn học</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">Mô tả</th>
                                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-200 uppercase tracking-wider">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800" wire:key="tbody-{{ $selectedProgramId }}">
                                @forelse ($subjects as $subject)
                                    <tr wire:key="subject-{{ $subject->id }}"
                                        class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
                                        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 text-center">{{ $subject->ordering }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 font-medium">{{ $subject->name }}</td>
                    ...
                    @if($subjects->hasPages())
                        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-800" wire:key="pagination-{{ $selectedProgramId }}">
                            {{ $subjects->links() }}
                        </div>
                    @endif
                </div>
