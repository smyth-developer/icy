                ...
                <table class="w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                        @forelse ($banks as $bank)
                            <tr wire:key="bank-{{ $bank->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 font-medium">{{ $bank->bank_name }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-300">{{ $bank->account_number }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-300">{{ $bank->account_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="px-2 py-1 rounded-full text-xs {{ $bank->status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-800/60 dark:text-gray-300' }}">
                                        {{ $bank->status === 'active' ? 'Hoạt động' : 'Ngưng' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <flux:button size="sm" variant="primary" icon="square-pen" wire:click="editBank({{ $bank->id }})" class="cursor-pointer">Sửa</flux:button>
                                        <flux:button size="sm" variant="danger" icon="trash" wire:click="deleteBank({{ $bank->id }})" class="cursor-pointer">Xoá</flux:button>
                                    </div>
                                </td>
                            </tr>
                        @empty
